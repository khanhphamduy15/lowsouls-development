...
namespace LS {
    ...
    {
        [SerializeField] AICharacterManager zombieCharacter;
        ...
        protected override void DamageTarget(CharacterManager dmgTarget)
        {
            ...
            damageEffect.angleHitFrom = Vector3.SignedAngle(zombieCharacter.transform.forward, dmgTarget.transform.forward, Vector3.up);

            if (dmgTarget.IsOwner)
            {
                dmgTarget.characterNetworkManager.NotifyTheServerOfCharacterDamageServerRpc(
                    dmgTarget.NetworkObjectId,
                    zombieCharacter.NetworkObjectId,
